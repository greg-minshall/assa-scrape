...
#+name: sections
| tag                | section     | premarker        | postmarker |
|--------------------+-------------+------------------+------------|
| dayHeader          | day         | **               |            |
| sessionTime        | time        | ***              |            |
...
#+BEGIN_SRC python :session py :var fname="aea-2016-assa-prelim.html" :var sections=sections
  import re
  ...
  def in_day(cur):
      ...
      if cur_day != rstring(cur):
          cur_day = rstring(cur)
          print premarker('dayHeader'), cur_day, postmarker('dayHeader')
  ...
  def in_time(cur):
      ...
      if cur_time != rstring(cur):
          ...
          print premarker('sessionTime'), cur_time, postmarker('sessionTime')
  ...
  def end_session():
      if 'sessionTitle' in session:
          print premarker('sessionTitle'), session['sessionTitle'].contents[0].strip(), postmarker('sessionTitle')
          if firstchild(session['sessionTitle']) :
              print premarker('sessionJEL'), firstchild(session['sessionTitle']).string.strip(), postmarker('sessionJEL')
      else:
          print premarker('sessionTitle'), "<unnamed session>", postmarker('sessionTitle')
      print premarker("sessionSource"), session['sessionSource'].string.strip(), postmarker('sessionSource')
      if 'sessionType' in session:
          print premarker('sessionType'], session['sessionType'].string.strip(), postmarker('sessionType')
      print premarker('sessionLocation'), re.sub("^,", "", session['sessionLocation'].string.strip()), postmarker('sessionLocation')
      print premarker('nameaffil'), nameaffils(session['presiding']), postmarker('nameaffil')

  def premarker(sect):
      return sections[sect][1]

  def postmarker(sect):
      return sections[sect][2]

  def withmarkers(sect, str):
      print premarker(sect), str, postmarker(sect)
  ...
  def end_paper():
      for paper in papers:
          ...
          print premarker('paperTitle'), title
          print premarker('nameaffil'), nameaffils(author)
          print premarker('hyperlink')
          ...
          for dsection in postsession:
              for d in dsection.children:      # discussant
                  if not navigablestring(d):
                      ...
                      print premarker('discussantsSection'), "discussant: %s" % nameaffils(d)
  ...
  # walk(soup.find(class_="dayHeader"))
#+END_SRC
