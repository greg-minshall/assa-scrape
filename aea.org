...
#+BEGIN_SRC python :session py :var fname="aea-2016-assa-prelim.html" :var sections=sections
  ...
  def in_day(cur):
      ...
      if cur_day != rstring(cur):
          cur_day = rstring(cur)
          print marker('dayHeader'), cur_day
  ...
  def in_time(cur):
      ...
      if cur_time != rstring(cur):
          ...
          print marker('sessionTime'), cur_time
  ...
  def end_session():
      if 'sessionTitle' in session:
          print marker('sessionTitle'), session['sessionTitle'].contents[0].strip()
          if firstchild(session['sessionTitle']) :
              print marker('sessionJEL'), firstchild(session['sessionTitle']).string.strip()
      else:
          print marker('sessionTitle'), "<unnamed session>"
      print marker("sessionSource"), session['sessionSource'].string.strip()
      if 'sessionType' in session:
          print marker('sessionType'], session['sessionType'].string.strip()
      print marker('sessionLocation'), re.sub("^,", "", session['sessionLocation'].string.strip()
      print marker('nameaffil'), nameaffils(session['presiding'])
  ...
  def end_paper():
      for paper in papers:
          ...
          print marker('paperTitle'), title
          print marker('nameaffil'), nameaffils(author)
          print marker('hyperlink')
          ...
          for dsection in postsession:
              for d in dsection.children:      # discussant
                  if not navigablestring(d):
                      ...
                      print marker('discussantsSection'), "discussant: %s" % nameaffils(d)
